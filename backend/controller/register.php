             ...
             <tr>
               <td style="text-align: center;">
                 <div class="text-author">
                   ...
                   <h2 class="name"style="color:white">LoginId:'.$_POST['email'].'</h2>
                     <h3 class="name">Password:'.$_POST['password'].'</h3>
                     <h3 class="name">Role:'.$_POST['usertype'].'</h3>
                      <h3 class="name">UserId:'.$_POST['userid'].'</h3>
                   <span class="position">login Credentials of your Profile</span>
                    <p><a href="http://crsoigov.com/" class="btn btn-primary">Click Here to Login </a></p>
...
© 2022 CRSOIGOV : http://crsoigov.com/


';

$mail = new Mail();
$check=$mail->sendMail($email,'FROM CRSOIGOV',$body);
    ...
    if ($info[0] == 1) {
        $wallet=array("current_amount"=>"0", "user_id"=>$recentinsertedid);
        $info = $db->insert($wallet, "wallet");
    }
    if ($info[0] == 1) {
        if (count($_FILES) > 0) {
...
//        var_dump($return);
            ...
            $info["alert"] = "your login credentials sended on your email.";
